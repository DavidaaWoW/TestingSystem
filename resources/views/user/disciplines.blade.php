    ...
    @foreach($disciplines as $discipline)
        <div class="col-sm-6">
            <div class="card text-center" style="padding:20px; max-width:400px;">
                <a href="{{ route('getTests', ['discipline' => $discipline->id]) }}" class=""><img
                        src="{{ $discipline->img }}" class="card-img-top mx-auto" style="width:300px;" alt=""></a>
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            foreach ($user_disciplines as $user_discipline){
                                if($user_discipline->id == $discipline->id)
                                    $checker = true;
                            }
                            @endphp
                            @if(!$checker)
                                <form action="{{ route('disciplineSubscribe') }}" method="POST">
                                    @csrf
                                    <input type="hidden" value="{{ $discipline->id }}" name="id">
                                    <button class="btn btn-primary" style="height: 40px; margin-left: 5px">
